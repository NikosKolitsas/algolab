...
inline bool is_solvable(vector<vector<int> >& healthy   , vector<vector<int> >& tumor, int degree ){
	int h = healthy.size();
	int t = tumor.size();
...
f f f t t t t
edw to hi deixnei panta sto prwto t
*/
inline int binary_search(vector<vector<int> >& healthy   , vector<vector<int> >& tumor,
		  int lo, int hi ){
	//dyadikh anazhthsh
	...
	solved[hi] = 1;  //thn kaloume mesw ths binary_search_exp_growth. ara sigoura to teleytaio true
			//kai afou yparxei sigoura toulaxiston mia lysh tote 8a mou epistrepsei ayth kai den
			//yparxei logos na eleksw sto telos an to  lo einai lysh
	int mid;
...
inline int binary_search_exp_growth(vector<vector<int> >& healthy   , vector<vector<int> >& tumor ){
