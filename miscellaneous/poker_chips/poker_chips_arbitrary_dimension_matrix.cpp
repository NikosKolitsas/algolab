...
void resize_vector(vector<long long>&, vector<int>& );
long long access_vector(vector<long long>&, vector<int>&, vector<int>& );
void store_vector(vector<long long>&, vector<int>&, vector<int>&, long long);
long long stored(vector<int> );

//aux functions
void print_vector(vector<int> );
void print_vector(vector<long long> );

/*ta kanw public tous pinakes aytous oratoi apo pantou*/
vector<vector<int> > colour;	//the colour of each poker chips
vector<long long> store_table;	//here we store the results.
...
long long recursion(vector<int> problem_state){
...
//	   cout << "\n new problem state size()= " << sbhsto.size() << endl;
		long long rec_score = stored( new_problem_state(problem_state, colour_map.second) ); 
		long long extra_score = score[colour_map.second.size()];  //σκορ αναλογο με το πληθος chips που αφαιρουμε
...
int main ( void ) {
	...
	long long cases;		//plhthos peiramatwn
	cin >> cases ;
...
void resize_vector(vector<long long>& myvector, vector<int>& size_of_dimensions){
	long long mult=1;
...
long long access_vector(vector<long long>& myvector, vector<int>& N, vector<int>& access_coord){
	int n = N.size(); //πληθος στηλων
	long long offset = access_coord[0];
	for(int i=1; i<n; i++){
...
void store_vector(vector<long long>& myvector, vector<int>& N, vector<int>& access_coord, long long value){
	int n = N.size(); //πληθος στηλων
	long long offset = access_coord[0];
	for(int i=1; i<n; i++){
		offset = offset*N[i] + access_coord[i];
...
long long stored(vector<int> problem_state){
	long long result = access_vector(store_table, dimensions, problem_state);
...
void print_vector(vector<long long> vector){
        for(auto v : vector)
                cout << v << ", ";
}
