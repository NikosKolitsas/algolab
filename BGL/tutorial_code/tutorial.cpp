...
/*	dijkstra_shortest_paths(G, t,
		predecessor_map(make_iterator_property_map(pred.begin(),
			...
			get(vertex_index, G))));
*/
/*	isodynamos me ton apopanw tropo!!!
	dijkstra_shortest_paths(G, t, distance_map(&dist[0]). predecessor_map(&pred[0]) );
*/
	/* kai ayto swsto. edw bazw mono to Map pou xreiazomai kai oxi ta ypoloipa   */
	dijkstra_shortest_paths(G, t, distance_map(&dist[0]) );
	int res = INT_MAX;
	for (int u = 0; u < V; ++u)
